...
text \<open>
  Selection does not depend on the order of the elements.
\<close>
...
proof -
  ...
  also have "\<dots> = (if k < length ys then select k ys else select (k - length ys) zs)" using assms(2)
    by (intro select_append') (auto simp: ys_def zs_def sum_length_filter_compl)
...
function fast_select where
  "fast_select k xs = (
     if length xs \<le> 23 then
       ...
       let x = fast_select (((length xs + 4) div 5 - 1) div 2) (map median (chop 5 xs));
...
proof induction
  ...
  next
    case False
    define x where
      "x = fast_select (((length xs + 4) div 5 - Suc 0) div 2) (map median (chop 5 xs))"
...
next
  ...
  define x where "x = fast_select (((length xs + 4) div 5 - 1) div 2) (map median (chop 5 xs))"
  assume A: "\<not> length xs \<le> 23" 
            "(ls, es, gs) = threeway_partition x xs"
